...
export default function IssueReportForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // Log what createIssue resolved to, so we can confirm success/failure and payload shape
    console.log("[IssueReportForm] createIssue result:", result);
  ...
  if (success) {
    return (
      <div className="w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-8 shadow-sm text-center">
        ...
        <h2 className="text-lg font-semibold text-ink">Issue created</h2>
        <p className="mt-1 text-sm text-gray-500">
          GitHub issue #{success.issueNumber} has been filed.
        </p>

        {success.mediaUrls.length > 0 && (
          <div className="mt-5 grid grid-cols-3 gap-2">
            {success.mediaUrls.map((url, index) => (
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-ink transition hover:bg-gray-50"
          >
